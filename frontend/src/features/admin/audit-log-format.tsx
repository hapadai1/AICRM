/**
 * 감사로그 표시 계층 (2026-08-05 AuditLogPage에서 분리).
 * 로그 한 건을 사람이 읽는 문장·라벨·전후 비교표로 바꾸는 순수 함수들과 DiffView.
 * 화면 상태를 갖지 않는다 — 페이지는 검색·표·드로어만 남긴다.
 */
...
export const ENTITY_MENU_LABELS: Record<string, string> = {
  USER: '관리자 > 사용자·권한',
...
export const CHANGES_INLINE_LIMIT = 3;

/**
 * 목록 한 줄에 들어갈 만큼 짧게 자른 값.
 * fmtValue는 배열을 줄바꿈으로 이어 붙이고 객체를 JSON으로 펼쳐 목록에서는 너무 길어진다.
 */
export function fmtInline(key: string, v: unknown, entityType: string): string {
...
export function changeMode(log: Pick<AuditLogItem, 'before' | 'after'>): ChangeMode {
 ...
 */
export function snapshotName(snapshot: Record<string, unknown> | null, entityType: string): string | null {
...
export function nameKeySet(entityType: string): Set<string> {
 ...
 */
export function targetName(log: Pick<AuditLogItem, 'before' | 'after' | 'entityType'>): string | null {
 ...
 */
export function targetFlow(log: Pick<AuditLogItem, 'before' | 'after' | 'entityType'>): {
  from: string | null;
  to: string;
 ...
 */
export function changeSentence(log: Pick<AuditLogItem, 'action' | 'before' | 'after' | 'entityType'>): string {
  const typeLabel = ENTITY_TYPE_LABELS[log.entityType] ?? log.entityType;
...
export const SINGLE_COLUMN_TITLE: Record<'before' | 'after', string> = {
  before: '삭제 전 값',
  after: '등록된 값',
};
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <Space style={{ marginBottom: 8 }}>
        ...
        <Typography.Link style={{ fontSize: 12 }} onClick={() => setShowAll((v: boolean) => !v)}>
