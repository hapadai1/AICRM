/**
 * 셔츠 옵션 세트를 '셔츠 디자인' 상담 자료 기준 5단계·22선택지로 맞춘다.
 *
 * 정장(seed-suit-design-options.ts)과 같은 방식으로 **첫 버전(V1)을 제자리 갱신**한다.
 ...
 *
 * - 선택지 사진은 prisma/assets/shirt-design/*.jpg (상담 자료 이미지에서 잘라낸 것).
 *   추출은 assets/extract-shirt-design-images.py가 담당하며 원본이 바뀔 때만 다시 돌린다.
 * - V1 말고 다른 버전이 남아 있으면 그 세션들을 V1으로 옮기고 버전을 정리한다.
 * - 실행: npm run seed:shirt-design
 *         SHIRT_DESIGN_IMAGES_ONLY=1 npm run seed:shirt-design  (사진만 갱신)
 */
...
const MARKER = '디자인 상담 자료 기준 셔츠 옵션';
const ASSET_DIR = resolve(__dirname, 'assets/shirt-design');
...
/** 상담 자료 배치 그대로 — 카라 3줄(각 5), 커프스 1줄(5), 더블 커프스(2) */
const STAGES: StageSeed[] = [
  {
    code: 'SHIRT_COLLAR_SPREAD',
    name: '카라 벌림',
    choices: [
      { name: '레귤러 카라' },
      { name: '세미와이드' },
  ...
  {
    code: 'SHIRT_DOUBLE_CUFF',
    name: '더블 커프스',
    choices: [{ name: '더블소매 굴림' }, { name: '더블소매 사각' }],
  },
];
...
async function ensureFile(stageCode: string, slot: string): Promise<string> {
  ...
  if (!existsSync(source))
    throw new Error(
      `선택지 이미지가 없습니다: ${source}\n` +
        'assets/extract-shirt-design-images.py를 먼저 실행하세요.',
    );

  const buffer = readFileSync(source);
  const checksum = createHash('sha256').update(buffer).digest('hex');
  const originalName = `shirt-design_${stageCode}_${slot}.jpg`;
  ...
  const id = randomUUID();
  const storageKey = `option-choices/shirt-design/${id}.jpg`;
...
async function main(): Promise<void> {
  const set = await prisma.optionSet.findUnique({ where: { productCategory: 'SHIRT' } });
  if (!set) throw new Error('셔츠 옵션 세트가 없습니다. 기본 시드를 먼저 실행하세요.');

  // 사진만 갱신: 자산을 다시 뽑았을 때 버전을 새로 만들지 않고 파일 내용만 바꾼다.
  if (process.env.SHIRT_DESIGN_IMAGES_ONLY === '1') {
    let n = 0;
    for (const stage of STAGES) {
      for (const [i] of stage.choices.entries()) {
        await ensureFile(stage.code, CODES[i]);
        n += 1;
      }
    }
  ...
  for (const stage of STAGES) {
    for (const [i] of stage.choices.entries()) {
      imageIds.set(`${stage.code}_${CODES[i]}`, await ensureFile(stage.code, CODES[i]));
    }
  }
 ...
 * 단계는 순번으로, 선택지는 코드(A~E)로 짝지어 선택값을 그대로 살린다.
...
main()
  .catch((error) => {
    console.error('셔츠 디자인 옵션 시드 실패:', error);
    process.exitCode = 1;
  })
  .finally(async () => {
